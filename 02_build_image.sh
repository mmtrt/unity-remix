...
cp "${CHROOT_PATH}"/boot/vmlinuz-"$(ls $CHROOT_PATH/lib/modules | tail -1)" "${IMAGE_PATH}"/casper/vmlinuz
cp "${CHROOT_PATH}"/boot/initrd.img-"$(ls $CHROOT_PATH/lib/modules | tail -1)" "${IMAGE_PATH}"/casper/initrd
...
#define DISKNAME  Unity Remix 20.04 LTS - amd64
#define TYPE  binary
#define TYPEbinary  1
#define ARCH  amd64
#define ARCHamd64  1
